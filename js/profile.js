...
async function handlePhotoUpload(e) {
  ...
  if (!allowedTypes.includes(file.type)) {
    showMsg(msg, "Please choose a JPG, PNG, or WEBP image.");
    e.target.value = "";
    return;
  }
  if (file.size > 5 * 1024 * 1024) {
    showMsg(msg, "That image is too large — please pick one under 5MB.");
    e.target.value = "";
    return;
  }
  ...
  // One fixed filename per user (upsert) so re-uploading always overwrites
  // the previous photo instead of piling up orphaned files in the bucket.
  ...
  if (uploadError) {
    ...
    showMsg(
      ...
      /bucket not found/i.test(uploadError.message)
        ? "Photo uploads aren't set up yet — ask an admin to create the 'avatars' storage bucket (see sql/schema.sql)."
        : "Couldn't upload photo: " + uploadError.message
    );
    e.target.value = "";
    return;
  }
  ...
  const { error: updateError } = await supabaseClient
    .from("profiles")
    .update({ photo_url: bustedUrl, updated_at: new Date().toISOString() })
    .eq("id", currentUser.id);

  hint.textContent = originalHint;

  if (updateError) {
    showMsg(msg, "Photo uploaded but couldn't save to your profile: " + updateError.message);
    return;
  }

  currentProfile.photo_url = bustedUrl;
  document.getElementById("photo_url").value = bustedUrl;
  avatarEl.style.backgroundImage = `url(${bustedUrl})`;
  avatarEl.textContent = "";
  showMsg(msg, "Profile photo updated.", "ok");
}
